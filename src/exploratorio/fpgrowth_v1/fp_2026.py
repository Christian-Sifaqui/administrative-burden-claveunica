...
# ===================== CONFIG =====================
MAIN_FILE = "data/cu_user_client-test.zip"
MAPPING_FILE = "dim_integracion_cu.csv"

SHARD_DIR = "shards-test1_2025"
K_SHARDS = 1024
MAX_OPEN_FILES = 64

TRANSACTIONS_FILE = "transactions_fp-2025_test1.txt"  # una transacción por línea: "12 45 900"
STATS_FILE = "transactions_stats-2025_test1.json"

FINAL_OUTPUT = "final_output-2025_test1.jsonl"
TRANSLATED_OUTPUT = "final_output_traducido-2025_test1.jsonl"
...
SKIP_MINING_IF_OUTPUT_EXISTS = False  # pon True para no minar si FINAL_OUTPUT ya existe
...
def check_python_implementation():
    """Verifica y muestra qué implementación de Python se está usando"""
    log = _get_logger()
    ...
    if impl_name == 'pypy':
        log("=" * 70)
        log("✅ EJECUTANDO CON PyPy - ¡Optimización JIT activada!")
        log(f"   Versión: {version}")
        log("=" * 70)
    elif impl_name == 'cpython':
        log("=" * 70)
        log("⚠️  EJECUTANDO CON Python normal (CPython)")
        log(f"   Versión: {version}")
        log("   💡 TIP: Usa 'pypy3 fp_2026_verificado.py' para 3-4x más velocidad")
        log("=" * 70)
    else:
        log("=" * 70)
        log(f"ℹ️  Implementación: {impl_name}")
        log(f"   Versión: {version}")
        log("=" * 70)
...
def compute_stats_from_transactions(transactions_file: str):
    """
    Calcula avg_apps / max_apps / N en una pasada, sin reescribir archivo.
    """
    total_apps = 0
    max_apps = 0
    n_tx = 0
...
# ===================== FASE 1: ZIP -> shards (run, item_idx) =====================
def preprocess_top_zip_to_shards(main_zip_path: str, shard_dir: str, cid_to_idx: dict, k: int):
    ...
    with zipfile.ZipFile(main_zip_path, "r") as top_zip:
        ...
        for inner_name in tqdm(inner_zip_names, desc="Leyendo ZIPs internos", dynamic_ncols=True):
            try:
...
def build_fptree_from_file_onepass_fast(transactions_file: str, header_counts: dict):
    """
    Fix Issue 2:
      - NO usar map(int, ...) con try/except afuera, porque es lazy.
      - Convertir a lista dentro del try para robustez.
    Nota: el archivo ya está filtrado a L1, así que no se chequea item in header.
    """
    log = _get_logger()
...
def fpgrowth_global_no_matrix(transactions_file: str, header_counts: dict, min_count: int, zmin: int, zmax: int):
    ...
    log("🌲 Construyendo FP-tree global (1 pasada, optimizado + robusto)...")
...
if __name__ == "__main__":
    ...
    # ✨ VERIFICACIÓN DE IMPLEMENTACIÓN (PyPy vs Python)
    check_python_implementation()

    log("🚀 Pipeline: FP-Growth global optimizado sin matriz (resume + stats fix + robust parsing)...")
    ...
    # 1) Sharding — saltar si ya existen
    if SKIP_SHARDING_IF_EXISTS and shards_exist(SHARD_DIR, K_SHARDS):
        log(f"♻️ Shards ya existen en '{SHARD_DIR}' (≥{K_SHARDS}). Saltando sharding.")
    else:
        preprocess_top_zip_to_shards(MAIN_FILE, SHARD_DIR, cid_to_idx, k=K_SHARDS)
    log_system_usage("Después de sharding:")

    # 2) Pass 1: conteos globales (N y soporte por item)
    log("🔢 Pass 1: contando soporte global por item (desde shards)...")
    ...
    log(f"✅ N usuarios (transacciones): {N:,}")
    ...
    # Guardrail #1: si L1 es enorme, subimos support SIN recontar shards
    ...
    if SKIP_TRANSACTIONS_IF_EXISTS and os.path.exists(TRANSACTIONS_FILE):
        ...
        if n_lines > 0:
            ...
            if stats and int(stats.get("N", -1)) == n_lines:
                ...
                log("📊 Calculando stats desde transactions_fp.txt (1 pasada)...")
        ...
        log("🧾 Pass 2: escribiendo transacciones filtradas (formato rápido)...")
        avg_apps, max_apps, N_written = write_transactions_file_from_shards(
            shard_dir=SHARD_DIR,
            transactions_file=TRANSACTIONS_FILE,
            header_counts=header_counts,
    ...
    # Guardrail #2: densidad (ahora SIEMPRE con stats en resume)
    zmax_eff = ZMAX
