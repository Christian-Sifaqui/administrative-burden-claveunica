...
def transaction_generator(temp_files):
    for i, path in enumerate(temp_files):
        print(f"[{i+1}/{len(temp_files)}] Cargando bloque desde {path}")
        report_memory_usage(f"Carga parcial {i+1}")
...
def save_frequent_itemsets_to_csv(freq_itemsets, id_name_mapping, output_file="frequent_itemsets.csv"):
    import csv
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Tamaño", "Conjunto", "Soporte"])
        for k, itemsets in freq_itemsets.items():
            for itemset, support in itemsets.items():
                names = [id_name_mapping.get(i, f"{i} de Desconocido") for i in itemset]
                writer.writerow([k, "; ".join(names), support])
...
def main():
    ...
    transactions = transaction_generator(temp_files)
    ...
    save_frequent_itemsets_to_csv(frequent_itemsets, id_name_mapping)
