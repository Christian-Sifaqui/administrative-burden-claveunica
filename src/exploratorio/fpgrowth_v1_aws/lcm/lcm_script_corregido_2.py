...
# FIX 3: dos archivos de metadata separados
SHARD_METADATA_FILE = "lcm_shard_metadata_2024_2025.json"    # solo parámetros de sharding
WEIGHTED_METADATA_FILE = "lcm_run_metadata_2024_2025.json"   # todos los parámetros
...
GUARD_MAX_UNIQUE_TX = 9_000_000
GUARD_MAX_WEIGHTED_DB_ROWS = 9_000_000
...
# FIX 3: metadata de sharding — solo lo que afecta a los shards
def build_shard_metadata(main_files, mapping_file, k_shards, num_items):
    return {
...
# FIX 3: metadata completa — incluye support/zmin/zmax/tx_buckets
def build_weighted_metadata(main_files, mapping_file, k_shards, tx_buckets,
                            support, zmin, zmax, num_items):
    return {
        **build_shard_metadata(main_files, mapping_file, k_shards, num_items),
...
def preprocess_multiple_zips_to_shards(main_zip_paths, shard_dir: str, cid_to_idx: dict, k: int):
    # FIX 1: limpiar directorio antes de escribir para evitar acumulación entre runs
    if os.path.isdir(shard_dir):
        log(f"🧹 Limpiando directorio de shards previo: {shard_dir}")
...
def build_tx_bucket_files_from_shards(
    ...
    # FIX 1: limpiar directorio antes de escribir para evitar acumulación entre runs
    if os.path.isdir(tx_bucket_dir):
        log(f"🧹 Limpiando directorio de tx_buckets previo: {tx_bucket_dir}")
        shutil.rmtree(tx_bucket_dir)
...
if __name__ == "__main__":
    ...
    # FIX 3: metadata separada por capa
    current_shard_meta = build_shard_metadata(
        main_files=MAIN_FILES,
        mapping_file=MAPPING_FILE,
        k_shards=K_SHARDS,
        num_items=num_items,
    )
    current_weighted_meta = build_weighted_metadata(
        main_files=MAIN_FILES,
        mapping_file=MAPPING_FILE,
        k_shards=K_SHARDS,
        tx_buckets=TX_BUCKETS,
    ...
    saved_shard_meta = load_json(SHARD_METADATA_FILE)
    saved_weighted_meta = load_json(WEIGHTED_METADATA_FILE)

    can_reuse_shards = (
        saved_shard_meta is not None
        and saved_shard_meta == current_shard_meta
    )
    can_reuse_weighted = (
        saved_weighted_meta is not None
        and saved_weighted_meta == current_weighted_meta
    )

    if saved_shard_meta is None:
        log("ℹ️ No hay metadatos previos de sharding.")
    elif can_reuse_shards:
        log("♻️ Shards compatibles con la configuración actual.")
    else:
        log("⚠️ Parámetros de sharding cambiaron → se rehará el sharding.")

    if can_reuse_shards and not can_reuse_weighted:
        log("⚠️ Parámetros de minería cambiaron (support/zmin/zmax/tx_buckets) → "
            "se rehará Pass 2 sin reetapar el sharding.")

    # 1) Sharding multi-zip
    preprocess_stats = None
    if SKIP_SHARDING_IF_EXISTS and can_reuse_shards and shards_exist(SHARD_DIR, K_SHARDS):
        log(f"♻️ Shards ya existen en '{SHARD_DIR}' (≥{K_SHARDS}) y son compatibles. Saltando sharding.")
    else:
        preprocess_stats = preprocess_multiple_zips_to_shards(
            MAIN_FILES, SHARD_DIR, cid_to_idx, k=K_SHARDS
        )
        save_json(SHARD_METADATA_FILE, current_shard_meta)
        # shards nuevos invalidan weighted DB
        can_reuse_weighted = False
    ...
    if (
        ...
        save_json(WEIGHTED_METADATA_FILE, current_weighted_meta)
        log(
    ...
    # FIX 2: mensaje de error honesto — no sugiere subir support porque eso no ayuda
    if unique_tx > GUARD_MAX_UNIQUE_TX:
        ratio = unique_tx / weighted_stats["N"]
        raise RuntimeError(
            f"La weighted DB tiene {unique_tx:,} transacciones únicas "
            f"(guardrail: {GUARD_MAX_UNIQUE_TX:,}, ratio de compresión: {ratio:.2f}). "
            f"Este dataset tiene baja repetición transaccional; subir el support no garantiza reducir "
            f"las únicas. Opciones: reducir ZMAX (actual: {zmax_eff}), "
            f"o usar un minador externo (SPMF/Java) que no requiera cargar en RAM."
        )
    ...
    if len(weighted_db) > GUARD_MAX_WEIGHTED_DB_ROWS:
        raise RuntimeError(
            ...
            f"{GUARD_MAX_WEIGHTED_DB_ROWS:,}. Reduce ZMAX o usa un minador externo."
        )
